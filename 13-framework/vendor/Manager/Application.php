...
final class Application
{
    ...
    public function __construct() {
        // On scrute(=regarder avec insistance) L-URL et on récupère les infos

        $tab = explode('/', $_SERVER['REQUEST_URI']);
        if(isset($tab[4]) && !empty($tab[4]) && file_exists(
            __DIR__ .'/../../src/Controller/' . ucfirst($tab[4]) . 'Controller.php')) {

                 $this -> controller = 'Controller\\' . ucfirst($tab[4]) . 'Controller';
        }
        else {
            //Dans le cas on lance la page par défaut. La home...
